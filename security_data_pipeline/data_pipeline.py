...
def process_budget(paths: List[Path], schema: Dict) -> pd.DataFrame:
    ...
    for p in paths:
        ...
        # Money columns coercion
        for col in df.columns:
            if any(x in col for x in ["amount", "cost", "budget", "expenditure", "expenses", "total"]):
                df[col] = (
                    df[col]
                    .astype(str)
    ...
    all_df = pd.concat(frames, ignore_index=True)
    keys = [k for k in ["location_name", "year", "budget_category"] if k in all_df.columns]
    agg = aggregate(all_df, keys)

    pivot = agg.pivot_table(index=[c for c in keys if c != "budget_category"],
                            columns="budget_category",
                            values=[c for c in agg.columns if c not in keys],
                            aggfunc="sum").reset_index()
    pivot.columns = ["_".join([str(c) for c in col if str(c) != ""]) for col in pivot.columns.to_flat_index()]

    # Heuristics to form canonical columns if present
    def pick_first(df, targets):
        for t in targets:
            if t in df.columns:
                return df[t]
        return np.nan

    if "amount_budget_expenditures" in pivot.columns:
        pivot["total_expenditure"] = pivot["amount_budget_expenditures"]
    if "amount_budget_operating" in pivot.columns:
        pivot["gross_budget"] = pivot["amount_budget_operating"]

    # Personnel total if present
    person_cols = [c for c in pivot.columns if "personnel_by_rank" in c and c.startswith("personnel_count")]
    if person_cols:
        pivot["total_personnel"] = pivot[person_cols].sum(axis=1)
